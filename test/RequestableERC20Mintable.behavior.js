...
function shouldBehaveLikeRequestableERC20Mintable (accounts, requestId = 0) {
...
function shouldBehaveLikeRequestableERC20MintableBasic (accounts, tokenAmount = toBN(1e18)) {
  const [owner, user1, user2, other] = toLowerCase(accounts);

  describe('RequestableERC20Mintable basic functions', function () {
    ...
    it('only minter can mint tokens', async function () {
      ...
      await this.token.mint(other, tokenAmount, { from: owner });
    });
